  ...
  if (reviewOrderBtn && creditCardForm) {
    reviewOrderBtn.addEventListener("click", function (e) {
      ...
      if (activeMethod && activeMethod.dataset.method === "credit-card") {
        ...
        if (
          !gcashNumber ||
          !gcashNumber.replace(/\s/g, "").match(/^09\d{9}$/)
        ) {
      ...
      setTimeout(async () => {
        ...
        showNotification(
          "Payment processed successfully! Finalizing order...",
          "success"
        );

        // Call backend to create order(s)
        try {
          ...
          if (resp.ok) {
            ...
            showNotification(
              body && body.error ? body.error : "Failed to create order",
              "error"
            );
            reviewOrderBtn.textContent = "PLACE ORDER";
